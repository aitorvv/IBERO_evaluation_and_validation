#------------------------------------------------------------------------------------------------#
#### Script elaborado para extraer la información de SIMANFOR y adaptarla al análisis con SAS ####
...
# creo los data.frame que recogeran la informaci?n final de ?rboles y parcelas

plots_IFN3 <- data.frame()
trees_IFN3 <- data.frame()


# creo una lista con el nombre de los archivos

files_list <- list.files(path = ".", pattern="xlsx")


### extraigo la informaci?n necesaria de cada archivo y la paso a los data.frames anteriores
...
# Renombrado de columnas tras la simulaci?n
...
# renombrar tambi?n los campos de las parcelas de primera ejecuci?n
...
### sumar los datos de masa muerta e incorporada de la primera ejecuci?n a la segunda

#### ?OJO! Esto se puede hacer as? porque tienen el mismo orden, sino hay que cambiar la forma de calcularlo
...
### Calcular ingrowth, mortality and harvested  on IFN3 ####

# calcular ingrowth, survive y cortados en IFN3, y marcar los ?rboles que corresponden en cada caso
...
# unir bases de datos en una sola, y luego con la BD original
...
#### Calcular incrementos dbh y h ####

# calcular los incrementos de ?rboles en dbh y h comparando con los datos originales del IFN2
...
#plots_IFN <- plots_IFN[!duplicated(plots_IFN),]
